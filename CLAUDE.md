...
- **תצוגת Reels:** הפיד הוא גלילה אנכית במסך מלא (scroll-snap), כותרת
  אחת בכל reel. **סדר כרונולוגי מדויק** (מהחדש לישן) בכל הטאבים,
  כולל "הכל" - `allItems` כבר ממוין ב-`fetch-news.mjs` לפני הכתיבה
  ל-`news.json`, ו-`render()` ב-`js/app.js` רק מסנן לפי מקור בלי
  לשנות סדר (הוסר `interleaveBySource` שערבב לפי מקור - לבקשת המשתמש
  ב-2026-09-16, כדי שהחדשות הכי עדכניות תמיד יופיעו ראשונות בלי קשר
  למקור). `.reel__time` מציג זמן יחסי טבעי בעברית - `עכשיו` / `לפני
  דקה` / `לפני X דקות` / `לפני שעה` / `לפני שעתיים` (צורת זוגי) / `לפני
  X שעות` / `לפני יום` / `לפני יומיים` (צורת זוגי) / `לפני X ימים`
  (`formatRelativeTime` ב-`js/app.js`). גלילה
  ...
  **גובה יציב מול toolbar נייד:** `html, body` משתמשים ב-`100svh` (לא
  `100%`/`100vh`) - `100%` עוקב בזמן אמת אחרי הופעה/היעלמות של סרגל
  הכתובת בדפדפן מובייל, ואם זה קורה תוך כדי גלילה, גובה כל `.reel`
  (שנגזר מהגובה הזה דרך flexbox) משתנה **אחרי** שנקודות ה-scroll-snap
  כבר חושבו - נראה כמו שני reels חצי-חצי על המסך בו-זמנית (דווח בפועל
  ע"י המשתמש, לא על כל reel - תלוי אם ה-toolbar בדיוק זז תוך כדי אותה
  גלילה). `100svh` (small viewport height - הגובה המינימלי כשה-toolbar
  גלוי במלואו) יציב ולא משתנה תוך כדי גלילה, אז אין דה-סינכרון.
...
- **מחיקת כותרת (swipe):** גלילה אופקית (ימינה או שמאלה, שתיהן - אותה
  ...
  המרחק עובר `SWIPE_DELETE_THRESHOLD_PX` (100) נפתח מסך אישור **מעוצב**
  (`#confirm-dialog` ב-`index.html`, לא `window.confirm` הדפדפני -
  הוחלף לבקשת המשתמש ל"עיצוב יותר מרשים") - מודל אמיתי עם אייקון פח
  אשפה (SVG), `role="alertdialog"`, `aria-modal`, לכידת פוקוס בין שני
  הכפתורים, Escape/קליק-רקע מבטלים, פוקוס פותח על "ביטול". `askConfirmDelete()`
  מחזיר `Promise<boolean>` (מוסתר רק אם `true`) - ביטול = חוזר למקום.
  עוצר/מפעיל autoplay מחדש סביב הדיאלוג (כמו כל מודל אחר בפרויקט). ההסתרה
  **מקומית למכשיר בלבד** (`localStorage`, מפתח `newsly:hiddenItemIds`,
  ...
  `preventDefault` ברמת `#reels` - delegation, לא משנה אם `.reel__link`
  הפנימי הוא ה-`<a>` בפועל). זו כוונה מוצהרת - אין UI נגיש-מקלדת חלופי
  לפתיחת המחיקה עצמה (המשתמש ביקש ספציפית swipe, לא כפתור על המסך) -
  אבל מסך האישור עצמו כן נגיש-מקלדת במלואו.
- **"מה הכותרת מסתירה?" (פענוח קליקבייט דרך Clickbyter):** כפתור
  (`.reel__decode`) **מעל** הכותרת בכל reel, שקורא **על פי דרישה
  בלבד** (לא אוטומטית לכל הפיד - יקר/איטי מדי, ראו PROJECT_STATE.md)
  ל-`POST https://clickbyter-api.ronmailx.workers.dev/api/decode` -
  ה-Worker (בפרויקט נפרד, `ronmailx-boop/clickbyter`) קורא את הכתבה
  המלאה מהמקור בצד שרת ומריץ Groq שמחזיר את מה שהכותרת "מסתירה"
  ב-1-3 משפטים. ה-CORS של ה-Worker כבר מוגדר לפי host בלבד
  (`ronmailx-boop.github.io`, לא path) אז זה עובד מ-Newsly בלי שום
  שינוי בצד clickbyter - **אין מפתח API בצד הלקוח**, ה-Worker הוא היחיד
  שמחזיק את מפתח ה-Groq. תוצאות (הצלחה/שגיאה) נשמרות ב-`Map` בזיכרון
  (`decodedAnswers`, לפי `item.id`) - זמני לסשן בלבד, לא `localStorage`
  - כדי לא לקרוא שוב ל-API לאותה כתבה אחרי מעבר טאבים/רינדור מחדש.
  מבנה `.reel__decode` הוא אלמנט אחות ל-`<a class="reel__link">`, לא
  מקונן בתוכו (כמו כפתור המחיקה). שגיאות (`FETCH_FAILED`,
  `EXTRACTION_FAILED` וכו') מתורגמות להודעה בעברית + "נסה שוב". מגבלה
  ידועה, **מאומתת בפועל**: כתבות מאתרים חסומי אנטי-בוט מחזירות
  `FETCH_FAILED` ולא יתפענחו - **ישראל היום ספציפית מוגן ע"י Akamai**
  (בדקתי ב-2026-09-14 עם workflow אבחון זמני בריפו clickbyter: קריאה
  ישירה לדף כתבה מ-israelhayom.co.il, גם מ-IP של GitHub Actions וגם
  מה-Worker, חוזרת 403 "Access Denied" מ-`AkamaiGHost`). **זה לא באג**
  באינטגרציה - זהה בעיקרון למקרה N12/Radware שכבר תועד: ה-RSS של ישראל
  היום עצמו פתוח לגמרי (ככה Newsly שולף ממנו כותרות בלי בעיה), אבל דף
  הכתבה המלא (מה ש-Clickbyter צריך לקרוא) חסום לתעבורה אוטומטית. לא
  לנסות לעקוף. **נסרקו כל 6 המקורות הפעילים** (2026-09-14, אותו workflow
  זמני) - **רק ישראל היום חסום** (403/AkamaiGHost); ynet, וואלה, מעריב,
  כיכר השבת והקול היהודי כולם מחזירים 200 לגישה ישירה לדף כתבה (200 לא
  מבטיח שה-AI יפיק תשובה טובה - רק שאין חסימת אנטי-בוט). **בעקבות זה
  (לבקשת המשתמש) הכפתור לא מוצג בכלל לכותרות מ"ישראל היום"** -
  `DECODE_BLOCKED_SOURCES` ב-`js/app.js` (Set של `sourceKey`, כרגע רק
  `israelhayom`) נבדק ב-`createReel` לפני קריאה ל-`buildDecodeBlock` -
  אם מקור עתידי יתגלה כחסום, מוסיפים אותו לשם. וה-Worker עצמו מוצהר
  (ב-PROJECT_STATE.md של clickbyter) כבלי rate limiting - שווה לזכור
  אם קצב השימוש עולה.
...
- **N12 - נוסף בפועל (2026-09-14), אבל דרך mako.co.il ולא n12.co.il:**
  n12.co.il עצמו עדיין חסום ע"י Radware Bot Manager (כולל ה-`/feed`
  שלו - מפנה ל-`validate.perfdrive.com`, עמוד אתגר של Radware) - **אין
  לנסות לעקוף את זה**. אבל N12 הוא בפועל "מאקו חדשות" שעבר מיתוג מחדש,
  והתוכן העיתונאי עדיין מוגש דרך תשתית `mako.co.il` הישנה - שם ה-RSS
  **פתוח ולא חסום**: `https://www.mako.co.il/rss/news-israel.xml`
  (נמצא ע"י ניחוש מבוסס-דפוס אחרי שדף אינדקס ה-RSS הרשמי של mako
  ב-`https://www.mako.co.il/rss/Article-64159d874b70221004.htm` חשף
  פיד קטגוריות תחת `rcs.mako.co.il/rss/...`, אבל ה-URL הזה עצמו לא היה
  ברשימה שם - התגלה בניחוש ישיר). אומת בפועל: XML תקין (`text/xml`,
  ~36KB), כותרת הפיד "חדשות - בארץ", כותרות/תאריכים כרונולוגיים
  ועדכניים (חדשות כלליות מהארץ, לא נישתי). **אבל** עמודי הכתבה המלאה
  תחת `mako.co.il` **כן חסומים** - אותה הגנת Radware בדיוק (`server:
  rdwr`, redirect ל-`validate.perfdrive.com`) - לכן `n12` נוסף גם ל-
  `DECODE_BLOCKED_SOURCES` ב-`js/app.js` (אותו טיפול כמו `israelhayom`
  מול Akamai: RSS פתוח לסינדיקציה, אבל הכתבה המלאה חסומה - כפתור פענוח
  הקליקבייט לא יעבוד שם ולכן מוסתר). ראו PROJECT_STATE.md לפרטים
  המלאים על מה שנבדק ונדחה (כלכליסט/גלובס/ביזפורטל/דה מרקר) לעומת N12
  שכן נוסף.
