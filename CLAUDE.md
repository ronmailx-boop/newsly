...
- **"טקסט מלא" בתוך ה-reel:** כפתור `.reel__expand-btn` פותח מסך תחתון
  (מודל אמיתי - `role="dialog"`, `aria-modal`, לכידת פוקוס בין שני
  האלמנטים הניתנים לפוקוס, Escape/קליק על ה-backdrop סוגרים, פוקוס חוזר
  לכפתור שפתח) שמציג את `item.summary` **במלואו** בלי לצאת לכתבה
  המקורית - עדיין יש קישור לכתבה המקורית בתוך המודל למי שרוצה. בכוונה
  **לא** מציג את הכתבה המלאה מהאתר עצמו (ראו הסבר תחת `data/news.json`
  למעלה) - רק את תקציר ה-RSS, שעכשיו לא חתוך ל-220 תווים כמו קודם. כפתור
  ה-"טקסט מלא" הוא אלמנט אחות (sibling) של `<a class="reel__link">`,
  לא מקונן בתוכו - `<button>` בתוך `<a>` הוא HTML לא תקין ובעייתי
  לנגישות. פותח/סוגר את המודל גם עוצר/מפעיל מחדש autoplay (כדי שהפיד לא
  יגלול הלאה מתחת למודל בזמן קריאה).
