...
- **מקורות `type: 'playwright'` (כמו N12):** נשלפים דרך `fetchPlaywrightSource`
  ב-`fetch-news.mjs` עם Chromium headless. אין תאריך פרסום אמין בעמודי
  בית - נעשה שימוש בזמן השליפה כ-`pubDate` (מספיק לצורך מיון/דה-דופ/גיזום).
  הסלקטור הוא היוריסטיקה גנרית (אורך טקסט, בלוקליסט ניווט, אותו דומיין)
  שדורשת כיוונון מול הלוגים האמיתיים של האתר - לא להניח שהיא מדויקת בלי
  לבדוק. ה-workflow חייב לכלול `npx playwright install --with-deps chromium`
  לפני הרצת הסקריפט.
