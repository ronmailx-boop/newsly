...
**אין כרגע משימה פתוחה** - הקוד נקי מהניסיונות האלה (הוסרו ה-tabs,
ה-dependencies, סקריפטי הבדיקה הזמניים). אם המשתמש ירצה לנסות שוב עם
כתובת RSS מדויקת (שהוא ימצא בעצמו בדפדפן), זו תוספת של שורה אחת
ב-`scripts/sources.mjs` + כפתור טאב ב-`index.html`.
